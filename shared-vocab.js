...
// ============================================================
// 🧹 XOÁ ĐÚNG CÁCH các "occurrences" (tham chiếu "từ này xuất hiện ở bài nào") khi 1 BÀI THẬT SỰ
// BỊ XOÁ — vấn đề trước đây: cơ chế tự khôi phục ở trên (vocabBankAutoHeal) chỉ biết CỘNG THÊM,
// không phân biệt được "thiếu do lỗi lưu" hay "thiếu do người dùng chủ động xoá bài" — nên dù đã
// xoá occurrence khỏi bản chính, lần mở app sau nó vẫn bị "hồi sinh" lại từ bản dự phòng trong
// IndexedDB. Hàm này XOÁ THẲNG (không hợp nhất) ở CẢ HAI nơi cùng lúc, nên không còn gì để tự
// khôi phục nhầm nữa. Gọi hàm này NGAY KHI XOÁ 1 BÀI (không phải khi xoá 1 thẻ từ nói chung).
// ============================================================
window.vocabBankPurgeOccurrences=function(bank,appLabel,deletedItemIds){
  const idSet=new Set(deletedItemIds||[]);
  if(!idSet.size) return bank;
  const cleaned=(bank||[]).map(rec=>{
    if(!rec||!Array.isArray(rec.occurrences)) return rec;
    const kept=rec.occurrences.filter(o=>!(o&&o.app===appLabel&&idSet.has(o.itemId)));
    if(kept.length===rec.occurrences.length) return rec;
    return Object.assign({},rec,{occurrences:kept});
  });
  try{
    openVocabIDB().then(db=>{
      const tx=db.transaction(VOCAB_IDB_STORE,"readwrite");
      tx.objectStore(VOCAB_IDB_STORE).put(cleaned,"bank");
    }).catch(e=>{});
  }catch(e){}
  return cleaned;
};

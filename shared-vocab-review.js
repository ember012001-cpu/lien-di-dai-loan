...
// ---- Gộp TẤT CẢ từ theo từng bài (KHÔNG lọc theo "cần ôn hôm nay") — dùng cho việc
// tải về/in bản đẹp để đưa lên AI khác làm bài tập, khi cần ĐẦY ĐỦ toàn bộ từ của 1 bài,
// không chỉ riêng phần "đến hạn ôn" (những từ vừa ôn xong hôm trước vẫn cần có mặt ở đây).
window.vocabGroupAllByItem=function(bank){
  const all=(bank||[]).filter(v=>v&&v.word);
  const map=new Map();
  all.forEach(v=>{
    const occs=Array.isArray(v.occurrences)?v.occurrences:[];
    if(!occs.length){

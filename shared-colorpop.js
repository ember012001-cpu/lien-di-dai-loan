      ...
      if(ed){
        ...
        if(_colorMode==='hl' && color==='transparent'){
          ...
          if(hasStructural){
            ...
            const startMark=document.createComment('_bgmarkS_'), endMark=document.createComment('_bgmarkE_');
            _savedRange.insertNode(endMark);
            _savedRange.insertNode(frag);
            _savedRange.insertNode(startMark);
            unwrapRedundantBgAncestors(startMark,endMark,ed);
          ...
          if(hasStructural){
            ...
            const frag=_savedRange.extractContents();
            // Xoá màu nền CŨ (nếu có) khỏi ngay bên trong phần vừa lấy ra, tránh việc mỗi lần
            // đổi màu lại chồng thêm 1 lớp <span> mới — đổi màu nhiều lần vẫn chỉ có 1 lớp.
            if(_colorMode==='hl') stripBackgroundColorDeep(frag);
            ...
            _savedRange.insertNode(startMark);
            if(_colorMode==='hl') unwrapRedundantBgAncestors(startMark,endMark,ed);
            startMark.remove(); endMark.remove();
          }
        }
...
// Sau khi đã xoá nền của phần chữ mình vừa chọn, trèo lên các thẻ cha (span cũ còn sót lại
// bọc bên ngoài) — nếu thẻ cha đó có màu nền VÀ toàn bộ nội dung của nó CHỈ LÀ đúng đoạn mình
// vừa xử lý (không "dính" thêm chữ nào khác), thì xoá luôn màu nền của thẻ cha đó, rồi tiếp
// tục trèo lên cao hơn. Nếu thẻ cha còn chứa thêm chữ khác không thuộc vùng đã chọn thì DỪNG
// lại ở đó, không đụng vào (để không lỡ xoá nhầm màu của phần chữ khác).
function unwrapRedundantBgAncestors(m1,m2,editableRoot){
  let el=m1.parentElement;
  while(el && el!==editableRoot && el.nodeType===1){
    const parent=el.parentElement;
    if(!parent) break;
    if(el.style && el.style.backgroundColor){
      // Kiểm tra bằng VỊ TRÍ trong cây DOM (2 điểm đánh dấu là con ĐẦU và con CUỐI của el,
      // không quan tâm thứ tự trước/sau) thay vì so sánh nội dung chữ — cách này vẫn đúng dù
      // bên trong có cấu trúc phức tạp (VD đang bật phiên âm, chữ bị bọc thêm <ruby><rt>...)
      // khiến so sánh text dễ bị sai lệch.
      const first=el.firstChild, last=el.lastChild;
      const matches=(first===m1&&last===m2)||(first===m2&&last===m1);
      if(matches) el.style.backgroundColor='';
      else break; // thẻ cha còn chứa thêm gì khác — dừng lại, không đụng vào
    }
    el=parent;
  }
}
